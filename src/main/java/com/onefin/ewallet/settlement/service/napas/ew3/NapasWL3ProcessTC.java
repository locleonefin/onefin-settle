...
public class NapasWL3ProcessTC extends QuartzJobBean {

	private static final Logger LOGGER = LoggerFactory.getLogger(NapasWL3ProcessTC.class);

	private static final String LOG_NAPAS_PG_PREFIX = SettleConstants.PARTNER_NAPAS + " " + SettleConstants.PAYMENT_GATEWAYWL3;

	private static final String NAPAS_PG_SUBJECT_NO_SETTLE = "[Napas - WL3] - Settle success";

	private static final String NAPAS_PG_SUBJECT_SETTLE = "[Napas - WL3] - Settle TC dispute";

	private static final String NAPAS_PG_ISSUE_SUBJECT = "[Napas - WL3] - Settle TC error";
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		...
		List<SettlementTransaction> transPendTCs = settleRepo
				.findAllByPartnerAndDomainAndStatusTransaction(SettleConstants.PARTNER_NAPAS, SettleConstants.PAYMENT_GATEWAYWL3, SettleConstants.SETTLEMENT_PENDING_T0);
		...
		for (SettlementTransaction transPendTC : transPendTCs) {
			...
			try {
				...
				jobTmp.setJobClass(NapasWL3ProcessTC.class.getName());
	...
	public void napasProcessTC(String napasEncode, String napasFileName,
		...
		for (String tmp : settleDateList) {
			...
			List<NapasEwalletEWTransactionView> transTmp1 = transRepository.findByEqualStatusAndNapasTransDate(
					configLoader.getNapasSettleAction(), SettleConstants.TRANS_SUCCESS, previousDate, configLoader.getSettleEwNapasBBB3());
			if (transTmp1.size() != 0) {
		...
		try {
			...
			OneFinInternal = new File(configLoader.getCeNapasDirectoryLocalFile() + "OFInternalWL3" + SLFileName);
			...
			for (NapasEwalletEWTransactionView tmp : settleNapasSuccess) {
				...
				if (check == false) {
					writer.write(this.createSLLine(tmp.getCardNumber(), tmp.getAmount(), tmp.getAmount(), tmp.getTransactionDate(),
							settleDate, configLoader.getSettleEwNapasTctvCode(), configLoader.getSettleEwNapasBBB3(),
	...
	private void updateTransactionList(List<String> settleDates, String status) throws Exception {
		for (String settleDate : settleDates) {
			SettlementTransaction trans = settleRepo.findByPartnerAndDomainAndSettleDate(SettleConstants.PARTNER_NAPAS,
					SettleConstants.PAYMENT_GATEWAYWL3, settleDate);
