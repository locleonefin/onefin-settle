...
public class ImediaTopupScheduleTask extends QuartzJobBean {

	private static final Logger LOGGER = LoggerFactory.getLogger(ImediaTopupScheduleTask.class);
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			taskEwalletImediaTopupSettlement(previousDate, dateRange);
		} catch (Exception e) {
	...
	public void scheduleTaskImediaTopupSettlementManually(Date dateInSettleMonth) throws Exception {
		Pair<Date, Date> dateRange = dateHelper.getDateRangeInMonth(dateInSettleMonth);
		taskEwalletImediaTopupSettlement(dateInSettleMonth, dateRange);
	}
	...
	public void taskEwalletImediaTopupSettlement(Date dateInSettleMonth, Pair<Date, Date> dateRange) throws Exception {
		LOGGER.info("== Start Imedia Bill Payment Settlement");
		// Find all SUCCESS transactions of previous month
		List<String> topupApiOperationList = Arrays.asList(SettleConstants.TOPUP, SettleConstants.BUYCARD);
		List<IMediaBillPayTransaction> result = transRepository.findByTranStatusAndApiOperationListAndFromToDate(SettleConstants.TRANS_SUCCESS, topupApiOperationList, dateRange.getFirst(), dateRange.getSecond());
		...
		try {
			...
			file = new File(configLoader.getSftpImediaTopupDirectoryStoreFile() + settleDate + "_" + SettleConstants.PARTNER_IMEDIA + "_" + SettleConstants.TOPUP + ".xlsx");
