...
public class SmartPosPaydiBunkUpload extends QuartzJobBean {

	private static final Logger LOGGER = LoggerFactory.getLogger(SmartPosPaydiBunkUpload.class);

	private static final String SMARTPOS_TRANS_FILE_NAME = "%s_SMARTPOS_TRANS_ONEFIN_PAYDI.txt";

	private static final String SMARTPOS_TRANS_RESULT_FILE_NAME = "%s_SMARTPOS_TRANS_RESULT_ONEFIN_PAYDI.txt";
	...
	public void taskBunkUploadProcess(DateTime currentTime) throws Exception {
		LOGGER.info("Start process smart pos bunk upload file {}", currentTime);
		...
		String bunkUploadFile = String.format(SMARTPOS_TRANS_FILE_NAME, previousDateString);
		LOGGER.info("Smart pos bunk upload file name {}", bunkUploadFile);
		...
		SettlementTransaction trx = settleRepo.findByPartnerAndDomainAndSettleDate(SettleConstants.PARTNER_PAYDI,
				SettleConstants.SMARTPOS_MERCHANT, new SimpleDateFormat(SettleConstants.DATE_FORMAT_yyyyMMdd).format(currentTime.toLocalDateTime().toDate()));
		if (trx == null) {
			trx = (SettlementTransaction) settleHelper.createModelStructure(new SettlementTransaction());
			trx.getSettleKey().setDomain(SettleConstants.SMARTPOS_MERCHANT);
		...
		try {
			...
			sftpVietinGateway.get(configLoader.getSftpPaydiRemoteIn() + bunkUploadFile, stream -> {
				...
				jobTmp.setJobClass(SmartPosPaydiBunkUpload.class.getName());
				...
				LOGGER.info("End smart pos bunk upload file {}", currentTime);
			});
		} catch (Exception e) {
			LOGGER.warn("Not Found smart pos bunk Upload File {}", previousDate, e);
		}
		...
		if (currentLocalTime.after(settleCompletedBeforeTime)) {
			if (processedBunkUpload.get().equals(false)) {
				LOGGER.error("Paydi not send smart pos bunk upload file!!!!");
			}
			SchedulerJobInfo jobTmp = new SchedulerJobInfo();
			jobTmp.setJobClass(SmartPosPaydiBunkUpload.class.getName());
			settleJobController.pauseJob(jobTmp);
	...
	private void processBunkUpload(String disputeFile, String date, SettlementTransaction finalTrx) {
		try (Stream<String> stream = Files.lines(Paths.get(disputeFile))) {
			File resultFile = new File(configLoader.getSftpPaydiStoreFile() + String.format(SMARTPOS_TRANS_RESULT_FILE_NAME, date));
			...
			stream.forEach((line) -> {
				try {
					...
					LOGGER.info("Smart pos trx details: {}", line);
			...
			LOGGER.error("Failed to process smart pos bunk upload file", e);
		}
	}
