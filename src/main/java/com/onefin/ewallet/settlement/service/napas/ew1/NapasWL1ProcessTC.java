package com.onefin.ewallet.settlement.service.napas.ew1;
...
public class NapasWL1ProcessTC extends QuartzJobBean {

	private static final Logger LOGGER = LoggerFactory.getLogger(NapasWL1ProcessTC.class);

	private static final String LOG_NAPAS_PG_PREFIX = SettleConstants.PARTNER_NAPAS + " " + SettleConstants.PAYMENT_GATEWAYWL1;

	private static final String NAPAS_PG_SUBJECT_NO_SETTLE = "[Napas - WL1] - Settle success";

	private static final String NAPAS_PG_SUBJECT_SETTLE = "[Napas - WL1] - Settle TC dispute";

	private static final String NAPAS_PG_ISSUE_SUBJECT = "[Napas - WL1] - Settle TC error";
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		...
		List<SettlementTransaction> transPendTCs = settleRepo
				.findAllByPartnerAndDomainAndStatusTransaction(SettleConstants.PARTNER_NAPAS, SettleConstants.PAYMENT_GATEWAYWL1, SettleConstants.SETTLEMENT_PENDING_T0);
		...
		for (SettlementTransaction transPendTC : transPendTCs) {
			...
			String settleFileName = String.format("%s_%s_%s_%s_%s_%s_%s.dat.pgp", transPendTC.getSettleKey().getSettleDate(),
					configLoader.getSettleEwNapasZZZ(), configLoader.getSettleEwNapasBBB1(),
			...
			try {
				...
				jobTmp.setJobClass(NapasWL1ProcessTC.class.getName());
	...
	public ResponseEntity<?> ewalletNapasTCSettlementManually(String date, String order,
		...
		String settleFileName = String.format("%s_%s_%s_%s_%s_%s_%s.dat.pgp", date, configLoader.getSettleEwNapasZZZ(),
				configLoader.getSettleEwNapasBBB1(), configLoader.getSettleEwNapasTctvCode(), order,
				configLoader.getSettleEwNapasSettleFileTypeTC(), configLoader.getSettleEwNapasServiceCodeEcom());
		try {
			sftpNapasGateway.get(configLoader.getSftpEwNapasDirectoryRemoteIn() + settleFileName, stream -> {
	...
	public void napasProcessTC(String napasEncode, String napasFileName,
		...
		for (String tmp : settleDateList) {
			...
			List<NapasEwalletEWTransactionView> transTmp1 = transRepository.findByEqualStatusAndNapasTransDate(
					configLoader.getNapasSettleAction(), SettleConstants.TRANS_SUCCESS, previousDate, configLoader.getSettleEwNapasBBB1());
			if (transTmp1.size() != 0) {
		...
		try {
			...
			OneFinInternal = new File(configLoader.getCeNapasDirectoryLocalFile() + "OFInternalWL1" + SLFileName);
			...
			for (NapasEwalletEWTransactionView tmp : settleNapasSuccess) {
				...
				if (check == false) {
					writer.write(this.createSLLine(tmp.getCardNumber(), tmp.getAmount(), tmp.getAmount(), tmp.getTransactionDate(),
							settleDate, configLoader.getSettleEwNapasTctvCode(), configLoader.getSettleEwNapasBBB1(),
			...
			try {
				...
				LOGGER.info("{} End Napas Settlement Processing TC File {}", LOG_NAPAS_PG_PREFIX, napasFileName);
			} catch (Exception e) {
	...
	private void updateTransactionList(List<String> settleDates, String status) throws Exception {
		for (String settleDate : settleDates) {
			SettlementTransaction trans = settleRepo.findByPartnerAndDomainAndSettleDate(SettleConstants.PARTNER_NAPAS,
					SettleConstants.PAYMENT_GATEWAYWL1, settleDate);
