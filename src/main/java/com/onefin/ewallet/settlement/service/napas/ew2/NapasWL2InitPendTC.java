...
public class NapasWL2InitPendTC extends QuartzJobBean {

    private static final Logger LOGGER = LoggerFactory.getLogger(NapasWL2InitPendTC.class);
    ...
    /**
     * @throws
     * Exception Create transaction with status "T0_PENDING" everyday to wait for Napas TC file (Thanh cong) uploading to sftp
     */
    @Override
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        SettlementTransaction trans = settleRepo.findByPartnerAndDomainAndSettleDate(SettleConstants.PARTNER_NAPAS,
                SettleConstants.PAYMENT_GATEWAYWL2, currentDate);
        if (trans == null) {
            try {
                trans = (SettlementTransaction) settleHelper.createModelStructure(new SettlementTransaction());

                trans.getSettleKey().setDomain(SettleConstants.PAYMENT_GATEWAYWL2);
